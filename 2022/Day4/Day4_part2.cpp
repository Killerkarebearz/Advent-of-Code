...
int twoEnd;
  for(i = 0;i<InputSize;i++)
  {
    ...
    /* === Determine Parial Overlaps ===*/
    // First Range Overlaps the Second //
    if((oneStart <= twoStart && oneEnd >= twoStart) || (oneEnd >=twoEnd && oneStart <= twoEnd))
      {  
      matches++;
      cout << "First Pair Overlap, running total: " << matches << endl;
      }
    //If the first pair overlapped, no need to check the reverse
    else if((twoStart <= oneStart && twoEnd >= oneStart) || (twoEnd >=oneEnd && twoStart <= oneEnd)) 
      {
      matches++;
